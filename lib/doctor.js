...
async function inspectRouter(host, password, options) {
  ...
  if (settings.requiredMenuView) {
    try {
      const menuResult = await rpcCall(
        host, session.sid, 'ui', 'get_menu_list', {}, transportOptions
      );
      report.plugin = {
        menu_view: settings.requiredMenuView,
        menu_loaded: menuContainsView(menuResult, settings.requiredMenuView),
      };
    } catch (error) {
      report.plugin = { menu_view: settings.requiredMenuView, menu_loaded: false };
      report.errors.push({ rpc: 'ui.get_menu_list', ...serializeError(error) });
    }
  }
  report.ok = compatibilityAccepted && (!report.plugin || report.plugin.menu_loaded);
  return report;
}
...
function formatDoctorReport(report) {
  ...
  if (report.plugin) {
    lines.push(
      `  Menu view ${report.plugin.menu_view}: ${report.plugin.menu_loaded ? 'loaded' : 'missing'}`
    );
  }
