...
async function doctor(args) {
  const parsed = parseRouterArgs(args || [], {
    allowJson: true,
    usage: 'Usage: glplugin doctor <host> [--https] [--insecure] [--username <name>] [--password-stdin] [--json]',
  });
  const password = await readRouterPassword({ passwordStdin: parsed.passwordStdin });
  const report = await inspectRouter(parsed.host, password, {
    ...authOptions(parsed),
