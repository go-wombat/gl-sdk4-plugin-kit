...
module.exports = function pkg() {
  ...
  try {
    ...
    // Build data.tar.gz
    execSync(
      `cd "${dataDir}" && tar --numeric-owner --group=0 --owner=0 -czf "${path.join(tmpDir, 'data.tar.gz')}" .`,
      { stdio: 'pipe' }
    );

    // Build control.tar.gz
    execSync(
      `cd "${controlDir}" && tar --numeric-owner --group=0 --owner=0 -czf "${path.join(tmpDir, 'control.tar.gz')}" .`,
      { stdio: 'pipe' }
    );
    ...
    // Try ar first, fall back to tar
    try {
      execSync(
        `cd "${tmpDir}" && ar r "${ipkFile}" debian-binary data.tar.gz control.tar.gz 2>/dev/null`,
        { stdio: 'pipe' }
      );
    } catch (e) {
      // Fallback: tar-based ipk (also valid)
      execSync(
        `cd "${tmpDir}" && tar --numeric-owner --group=0 --owner=0 -cf "${ipkFile}" ./debian-binary ./data.tar.gz ./control.tar.gz`,
        { stdio: 'pipe' }
      );
    }
