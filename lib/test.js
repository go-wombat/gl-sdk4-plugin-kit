 ...
 *
 * Usage: glplugin test <host> [--password-stdin]
 *
 * Tests:
...
const { login, call, normalizeRouterUrl } = require('./auth');
...
const { authOptions, parseRouterArgs } = require('./router-command');
...
module.exports = async function testRouter(args) {
  const parsed = parseRouterArgs(Array.isArray(args) ? args : [args].filter(Boolean), {
    usage: 'Usage: glplugin test <host> [--https] [--insecure] [--username <name>] [--password-stdin]',
  });
  const host = parsed.host;
  const password = await readRouterPassword({ passwordStdin: parsed.passwordStdin });
  const transportOptions = authOptions(parsed);
  ...
  try {
    const res = await httpGet(host, '/', transportOptions);
    if (res.status === 200) {
  ...
  let sid;
  try {
    const session = await login(host, password, parsed.username, transportOptions);
    sid = session.sid;
  ...
  for (const [mod, func] of KNOWN_METHODS) {
    try {
      const result = await call(host, sid, mod, func, {}, transportOptions);
  ...
  const manifestPath = path.resolve(process.cwd(), 'gl-plugin.json');
  let legacyPlugin = false;
  if (!fs.existsSync(manifestPath) && fs.existsSync(pkgPath)) {
    try {
      legacyPlugin = Boolean(JSON.parse(fs.readFileSync(pkgPath, 'utf8')).pluginName);
    } catch (error) {
      legacyPlugin = false;
    }
  }
  if (fs.existsSync(manifestPath) || legacyPlugin) {
    const name = readPluginProject(process.cwd()).manifest.id;
    if (name) {
      console.log('');
      try {
        const res = await httpGet(host, `/views/gl-sdk4-ui-${name}.common.js.gz`, transportOptions);
        if (res.status === 200) {
