...
async function verifyRouter(host, password, options) {
  ...
  try {
    doctor = await (settings.inspectRouter || inspectRouter)(host, password, {
      ...
      requiredMenuView: project && project.manifest.id,
    });
  } catch (error) {
  ...
  if (project) {
    const menuLoaded = Boolean(doctor.plugin && doctor.plugin.menu_loaded);
    add('plugin-menu', menuLoaded ? 'pass' : 'fail', menuLoaded
      ? `${project.manifest.id}: loaded by ui.get_menu_list`
      : `${project.manifest.id}: missing from ui.get_menu_list`);
  }
