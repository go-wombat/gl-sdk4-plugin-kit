...
function parseWorkflowArgs(args, options) {
  ...
  const parsed = { target: '', build: true, hostKeyPolicy: null };
  args.forEach((value) => {
...
function installPlugin(options) {
  ...
  return withSshSession(settings.target.ssh, (sessionOptions) => {
    ...
    sshRun(
      settings.target.ssh,
      `opkg install ${remotePath}; status=$?; rm -f ${remotePath}; exit "$status"`,
      sessionOptions
    );
...
function installCli(args, options) {
  const parsed = parseWorkflowArgs(args, {
    allowNoBuild: true,
    usage: 'Usage: glplugin install [target|host] [--no-build] [--insecure-host-key]',
  });
